...
# An instance caputres the point in time information for a barclamp 'configuration'.
class ConfigurationInstance < ActiveRecord::Base
  ...
  belongs_to      :configuration, :inverse_of => :instances
  has_many        :node_roles
  ...
  def add_node_to_role(node, role)
    nr = NodeRole.find_by_node_id_and_role_id_and_config_instance_id(node.id, role.id, self.id)
    unless nr
      nr = NodeRole.create
      nr.node = node
      nr.role = role
  ...
  def get_nodes_by_role(role_name)
    ...
    nrs = NodeRole.find_all_by_role_id_and_config_instance_id(role.id, self.id)
    answer = []
    nrs.each do |nr|
      answer << nr.node
    end
  ...
  def get_node_config_hash(node)
    nr = NodeRole.find_by_node_id_and_config_instance_id_and_role_id(node.id, self.id, nil)
    return {} unless nr
